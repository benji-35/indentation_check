...
void check_directory(char *path, int inden, list *l, int check_end, int *nb_inden, int *nb_space, int max_col, int *col, list *ext_o, int replace)
{
    struct dirent *entry;
    struct stat st;
    struct stat *_st;
    ...
    while (entry != NULL) {
        ...
        if (new_path != NULL) {
            ...
            if (S_ISDIR(_st->st_mode)) {
                check_directory(new_path, inden, l, check_end, nb_inden, nb_space, max_col, col, ext_o, replace);
                ...
                if (replace)
                    replace_tab(read, inden, new_path);
                free(read);
            }
            free(_st);
            free(new_path);
